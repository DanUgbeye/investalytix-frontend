...
export default function IndustrySectorComparisonScreen(
  ...
  const { ticker, currency, profile, sectorPerformanceHistory, similarStocks } =
    props;
  const { theme } = useTheme();
  const { ref, isScrolled } = useScroll<HTMLDivElement>();

  const sectorTableData = useMemo(() => {
    return generateSectorData(sectorPerformanceHistory);
            ...
            <Table className="w-full min-w-[50rem]">
              <TableHeader>
                <TableRow headerRow>
                  ...
                  {sectorPerformanceHistory.map((data, index) => {
                    return (
                      <TableHead
